...
def load_or_download_speaker(use_hf=True, speaker_id=None, speaker_path=None):
    if speaker_path is None:
        speaker_path = hf_hub_download(repo_id=CONVERT_TO_HF_REPO_ID, filename=f"base_speakers/ses/{speaker_id}.pth")
    return speaker_path